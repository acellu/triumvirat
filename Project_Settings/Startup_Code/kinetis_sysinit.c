...
void isr_PIT1(void){
	...
	if (PIT_TFLG1 == 1){              // if a time-out has occurred
		PIT_TFLG1 = 1;                // Acquitter le flag
		GPIOC_PTOR |= 0x100; //Toggle Yellow led
	}
...
void isr_portC(void){
	//SW3 Interrupt
	if((PORTC_PCR5 & PORT_PCR_ISF_MASK) == 1){
		PORTC_PCR5 &= ~PORT_PCR_ISF_MASK;
		
		GPIOC_PTOR |= 0x100;
	}
	//SW4 Interrupt
	else if(PORTC_PCR13 && PORT_PCR_ISF_MASK == 1){
...
static __declspec(vectortable) tVectorTable __vect_table = { /* Interrupt vector table */
   __SP_INIT,                                              /* 0 (0x00000000) (prior: -) */
  {
   ...
   (tIsrFunc)isr_PIT0,           	                       /* 84 (0x00000150) (prior: 0) PIT0 Interruption*/
   (tIsrFunc)isr_PIT1,                               /* 85 (0x00000154) (prior: -) */
   ...
   (tIsrFunc)isr_portC,                               	   /* 105 (0x000001A4) (prior: 0) */
